...
def load_wall():
    ...
    for i in range(N_rows):
        # layer of | | | | | | or vertical layers(better way of explaining this)
        ...
        for k in range(int(N_cols / 2)):
            xyz = p0 + np.array(
                [x_spacing * (2 * k + 0.5), 0, (brick_height + brick_width) * (i + 1) - brick_width / 2])
            rpy = np.array([np.pi / 2, 0, np.pi / 2])
            pose = array_to_pose(xyz, rpy)
            poses.append(pose)
            names.append('brick' + str(i) + str(k) + 'horizontal')
    print(zip(poses, names))
    for pose, name in zip(poses, names):
        ...
        with open("models/" + file + "/model.sdf", "r") as model_file:
            # load xml from SDF
            model_xml = model_file.read().replace('\n', '')
        rospy.wait_for_service('/gazebo/spawn_sdf_model', timeout=None)
        try:
            # spawn the model
            spawn_sdf = rospy.ServiceProxy('/gazebo/spawn_sdf_model', SpawnModel)
            resp_sdf = spawn_sdf(name, model_xml, "/",
...
def delete_gazebo_models():
    ...
    models = ["ball", "cafe_table_1", "cafe_table_2", "pick_plate", "place_plate",
              "middle_plate", "brick", 'sponge', 'sponge', 'brick_hit', 'brick_hit']
    models.extend(names)
    models.extend(names)
    models.extend(names)
...
class BaxterArm(object):
    ...
    def ik_request(self, pose):
        ...
        if (resp_seeds[0] != resp.RESULT_INVALID):  # check if the response is valid
            ...
            if self._verbose:
                print
                '[INFO] IK joint solution found!!!'
                print
                '========================= Joint Space Target State ===================='
                for joint in limb_joints:
                    print
                    joint + ': ', np.round(limb_joints[joint], 4)
                print("------------------")
            return limb_joints
        else:
    ...
    def gripper_open(self):
        """ Open the gripper """
        print
        '[ACTION] Open ' + self._limb_name + ' gripper...'
        self._gripper.open()
        rospy.sleep(1.0)
    ...
    def gripper_close(self):
        """ Close the gripper """
        print
        '[ACTION] Close ' + self._limb_name + ' gripper...'
        self._gripper.close()
        rospy.sleep(1.0)
    ...
    def servo_to_pose(self, xyz, rpy):
        ...
        'orientation:'
        print
        'roll:', np.around(rpy[0], 4)
        print
        'pitch:', np.around(rpy[1], 4)
        print
        'yaw:', np.around(rpy[2], 4)
        roll, pitch, yaw = rpy
        quaternion = tf.transformations.quaternion_from_euler(roll, pitch, yaw)
        print
        'quaternion:'
        print
        'x: ', np.around(quaternion[0], 4)
        print
        'y: ', np.around(quaternion[1], 4)
        print
        'z: ', np.around(quaternion[2], 4)
        print
        'w: ', np.around(quaternion[3], 4)
        print("------------------")
...
def main():
    ...
    rate.sleep()
    step = 0
    pullback = False  # iterating that the pullback procedure has not taken place yet
    ...
    while not rospy.is_shutdown():
        ...
        # external torque
        ############################ TASK H section
        #y_stop = -0.2 # the y position to release the brick from  as described but not defined in 6.H.ii.C1
        y_stop = 0.0
        y_pullback = 0.80 # the y position to pull the arm back to before throwing the brick

        external_force = np.array([0, 0, 0])  # force to accelerate the end effector
        force_pulback = np.array([0, 3, 1])  # force to pull the arm back
        force_push_forward = np.array([0, -10, 0])  # force to push the arm back as explained in figure 6.H.i.C1 and 6.H.i.1

        if y < y_pullback and pullback == False: # if the arm is not pulled back yet as described in  6.H.ii.2 and 6.H.ii.C2
            y0 = y_pullback # set the referance position to the pullback position
            external_force = force_pulback # set the external force to the pullback force
        elif y > y_pullback and pullback == False: # if the arm is pulled back
            y0 = y_stop # set the referance position to the stop position
            pullback = True # set the pullback flag to true
            external_force = np.array([-0.5, -1.6, 0.1]) # give the arn some time to stabalise
        else:
            external_force = force_push_forward # if the arm has been pulled back, push the arm forward
        ...
        perpendicular_position_error = position_error - external_force * np.dot(position_error,
                                                                                external_force) / np.dot(external_force,
                                                                                                         external_force)
        perpendicular_velocity_error = velocity_error - external_force * np.dot(velocity_error,
                                                                                external_force) / np.dot(external_force,
                                                                                                         external_force)
        ...
        if y < y_stop:
            ...
            external_force += 80.0 * np.array([- x_dot, - y_dot, - z_dot]).reshape((3, 1))
            external_torque = np.matmul(J[:3].T,
                                        external_force)  # just the position part of the jacobian (not orientation)
            break
    ...
    while not rospy.is_shutdown():
        ...
        # jacobian
        J = right_arm.kin.jacobian() # as described in eq 6.2
        ...
        if direction == 'right':
            ...
            external_force = np.array([0.0, 8, -20, 0, 0, 0]).reshape(
                (6, 1))  # force to push down and across on the table as shown in 6.I.i.C1 and 6.I.ii.C1
            ...
            external_force += 20.0 * np.array([- x_dot, - y_dot * 0.2, - z_dot, -4 * x_omega, -0.3 * y_omega, 0]).reshape(
                (6, 1))  # derivative feedback incorporating omega to account for rotationional error as shown in 6.I.ii.C1
        ...
        if direction == 'left':
            ...
            #
            external_force = np.array([0.0, -8, -20, 0, 0, 0]).reshape(
                (6, 1))  # force to push down and across on the table 6.I.i.C2 and 6.I.ii.C2
            # add proportional and derivative control to keep the end effector from deviating in the x direction
            external_force += 400.0 * np.array([0.75 - x, 0.0, 0.0, -0, -0, 0]).reshape((6, 1))  # proportional feedback
            external_force += 20.0 * np.array([- x_dot, - y_dot * 0.2, - z_dot, -4 * x_omega, -0.3 * y_omega, 0]).reshape(
                (6, 1))  # derivative feedback incorporating omega to account for rotationional error as shown in 6.I.ii.C2 

            # convert the external force that the end effector is applying to joint torques
            external_torque = np.matmul(J.T, external_force)

            # if the y position goes beyond the left hand limit, change direction
            # if y > y_left_stop or t > 4:
            if y < y_right_stop:
                direction = 'right'
                print
                'Wiping right'
